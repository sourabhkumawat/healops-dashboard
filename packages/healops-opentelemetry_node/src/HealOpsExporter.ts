...
export class HealOpsExporter implements SpanExporter {
    ...
    constructor(config: HealOpsConfig) {
        this.config = config;
        this.endpoint =
            config.endpoint || 'https://engine.healops.ai/otel/errors';
    }
    ...
    export(
        ...
        Promise.all(spans.map((span) => this.transformSpan(span)))
            ...
            .catch((error) => {
                console.error(
                    'Failed to export spans to HealOps:',
                    error.message
                );
        ...
        for (const event of span.events) {
            if (event.name === 'exception' && event.attributes) {
                ...
                if (event.attributes['exception.stacktrace']) {
                    ...
                    enhancedAttributes['exception.stacktrace'] =
                        exceptionStacktrace;
                    stackTrace = exceptionStacktrace;
                }
                if (event.attributes['exception.type']) {
                    enhancedAttributes['exception.type'] =
                        event.attributes['exception.type'];
                }
                if (event.attributes['exception.message']) {
                    enhancedAttributes['exception.message'] =
                ...
                if (extractedPath) {
                    ...
                    if (resolvedPath) {
                        ...
                        if (
                            ...
                            enhancedAttributes['code.column.number'] =
                                columnNum;
                        }
                        if (functionName) {
                            enhancedAttributes['code.function.name'] =
                                functionName;
                        }
