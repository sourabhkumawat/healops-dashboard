...
from src.memory.models import AgentMemoryError, AgentMemoryFix, AgentRepoContext, AgentLearningPattern
...
class CodeMemory:
    ...
    def _get_db_session(self) -> Session:
        """Get a new database session for an operation."""
        return SessionLocal()
    
    def _with_db_session(self, func):
        """Context manager helper for database operations."""
    ...
    def store_error_context(self, error_signature: str, context: str):
        ...
        def _store_in_db(db: Session):
            try:
                # Check if exists
                existing_error = db.query(AgentMemoryError).filter(AgentMemoryError.error_signature == error_signature).first()
                if existing_error:
        ...
        self._with_db_session(_store_in_db)
    ...
    def store_fix(self, error_signature: str, fix_description: str, code_patch: str, structured_data: Optional[Dict[str, Any]] = None):
        ...
        def _store_in_db(db: Session):
            try:
                ...
                if structured_data:
                    code_patch_json = json.dumps(structured_data)
                else:
                    code_patch_json = code_patch
                
                new_fix = AgentMemoryFix(
                    error_signature=error_signature,
                    description=fix_description,
                    code_patch=code_patch_json
                )
                db.add(new_fix)
                db.commit()

                # Invalidate/Update Redis cache for retrieval
                self._cache_fixes(error_signature, db)
    ...
    def _cache_fixes(self, error_signature: str, db: Session = None):
        """Helper to cache fixes in Redis."""
        if db is None:
            def _cache(db: Session):
                self._cache_fixes(error_signature, db)
            self._with_db_session(_cache)
            return
        
        try:
            fixes = db.query(AgentMemoryFix).filter(AgentMemoryFix.error_signature == error_signature).all()
            fixes_data = [
    ...
    def retrieve_context(self, error_signature: str) -> Dict[str, Any]:
        ...
        if not result["known_fixes"] or not result["past_errors"]:
            def _fetch_from_db(db: Session):
                ...
                if not result["past_errors"]:
                    error = db.query(AgentMemoryError).filter(AgentMemoryError.error_signature == error_signature).first()
                    if error:
            ...
            self._with_db_session(_fetch_from_db)

        return result

    def update_repo_context(self, file_path: str, summary: str):
        """Updates the memory with a summary of a file."""
        def _update_in_db(db: Session):
            try:
                existing = db.query(AgentRepoContext).filter(AgentRepoContext.file_path == file_path).first()
                if existing:
        ...
        self._with_db_session(_update_in_db)
    ...
    def _learn_from_workspace(
        ...
        def _learn_in_db(db: Session):
            ...
            pattern = db.query(AgentLearningPattern).filter(
        ...
        self._with_db_session(_learn_in_db)
    ...
    def get_learning_pattern(self, error_type: str) -> Optional[Dict[str, Any]]:
        ...
        self._with_db_session(_get_from_db)
        return result[0]
    ...
    def update_fix_success_rate(self, error_signature: str, success: bool):
        ...
        def _update_in_db(db: Session):
            try:
                fixes = db.query(AgentMemoryFix).filter(
